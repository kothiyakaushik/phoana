...
@section('title','Service Level-1 Sub Categories')
        ...
        <div class="box">
          ...
          <div class="box-header with-border">
            <div class="pull-right">
              <a href="{{route('getAdminAddnewSubCategory',Request::all())}}" class="btn btn-primary pull-right">Add New</a>
            </div>
          </div>
          ...
          <div class="box-body">
            ...
            <div class="form-group">
              ...
              {{Form::open(array('url' => route('postAdminListSubCategory'), 'method' => 'post','class'=>'row'))}}  
                <div class="form-row">
                ...
                <div class="col-md-12">
                  Show <select id="recordsPerPage" data-target="sub-category-listing">
                  ...
                  @if($isRequestSearch)
                    <a href="{{route('getAdminListSubCategories')}}" class="pull-right btn btn-danger btn-sm">Reset Search </a>
                  @endif
                  ...
                  <table class="table table-bordered table-striped table-hover">
                    <thead>
                      <tr>
                        ...
                        <th>Category Name <a href="{{route('getAdminListSubCategories', $sort_columns['category_name']['params'])}}"><i class="fa fa-angle-{{$sort_columns['category_name']['angle']}}"></i></a></th>
                        <th>Active/Inactive <a href="{{route('getAdminListSubCategories', $sort_columns['active']['params'])}}"><i class="fa fa-angle-{{$sort_columns['active']['angle']}}"></i></a></th>
                        <th>Action</th>
                      </tr>
                    </thead>
                    ...
                    @if(count($allSubCategories)!=0)
                      @foreach($allSubCategories as $category)
                        <tr>
                          ...
                          <td>
                            <a  href="{{route('getAdminEditSubCategory',array_merge( ['Category'=> $category->id ], Request::all()) )}}">{{$category->category_name}}</a>
                          </td>
                          <td>
                          ...
                          <td>
                            <a href="{{route('getAdminEditSubCategory',array_merge( ['Category'=> $category->id ], Request::all()) )}}" class="btn-success btn-sm">Edit</a>
                            <a onclick="return confirmDelete(event);" href="{{route('getAdminDeleteSubCategory',array_merge( ['Category'=> $category->id], Request::all()) )}}" class="btn-danger btn-sm">Delete</a>
                          </td>
                        </tr>
                      @endforeach
                    @else
